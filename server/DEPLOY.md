...
首次交互安装会先显示“必需的基础配置”，依次询问 TCP/UDP 共用端口和服务端鉴权密码，然后才询问是否进入可选的高级配置。端口留空使用 `11510`；密码留空会从 `/dev/urandom` 生成 64 位十六进制密码，并仅在成功安装后显示一次。不需要高级设置时直接回答 `N` 或按回车即可，端口和密码不会因此被跳过。

基础配置会在 GitHub tag 查询之前完成，因此即使后续访问 GitHub 失败，用户也能明确看到端口和密码配置步骤。通过管道执行脚本时会从 `/dev/tty` 读取交互输入；如果当前会话没有可用控制终端，安装器会显示警告并使用已有值或默认值。此时应使用 `--port` 和 `--password-file` 显式配置，或在带 TTY 的会话中运行。

脚本默认使用 `auto` 操作：未安装时执行安装；已有安装时更新到最新正式 tag，显示正在保留的端口和密码，但不会要求重新输入。使用 `--reconfigure` 才会依次重新配置端口、密码和可选高级设置。也可以显式指定操作:
